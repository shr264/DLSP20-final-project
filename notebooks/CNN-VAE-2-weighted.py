...
for epoch in range(0, epochs + 1):
    ...
    if epoch >= 0:  # test untrained net first
        ...
        for i, data in enumerate(trainloader):
            ...
            loss = loss_function(x_hat, x, mu, logvar, epoch)
    ...
    if test_loss_post < best_loss:
        ...
        torch.save(best_model.state_dict(), 
                   'models/CNNVAE_BB_model_weighted.pth')
    ...
    plt.savefig("imgs/CNNVAE_weighted_plot_epoch_"+str(epoch)+".png", dpi=150)
    plt.close(fig)
    
pd.DataFrame(accuracy_list).to_csv('CNNVAE_weighted_accuracy_list.csv')

torch.save(codes,"CNNVAE_weighted_codes.dict")
